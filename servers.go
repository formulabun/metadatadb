...
func (c *Client) AddServerFiles(host string, files []string, ctx context.Context) error {
...
func (c *Client) AddServerInfo(host string, info network.ServerInfo, ctx context.Context) error {
	col := c.getCollection(serversColl)
	country, region, err := ParseHost(host)
	if err != nil {
		return err
	}
	res := col.FindOneAndUpdate(
...
func ParseHost(host string) (string, string, error) {
	...
	return country, c.Region().String(), nil
}
